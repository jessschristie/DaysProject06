...
namespace DaysProject5.Controllers
{
    public class MoviesController : Controller
    {
        ...
        public ActionResult ReadFromSearch([DataSourceRequest] DataSourceRequest request, DateTime start, DateTime end)
        {
            DataSourceResult result = MovieServ.Read().Where( x => x.Release >= start && x.Release <= end ).ToDataSourceResult(request, Mov => new Movie
            {
                ID = Mov.ID,
                Title = Mov.Title,
                WeekendRevenue = Mov.WeekendRevenue,
